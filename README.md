...
| 기능 | 설명 | 담당자 |
|------|------|--------|
| [게시물 업로드](#post-upload) | 해시태그, 닉네임, 이미지를 포함한 게시글을 업로드하는 기능 구현. | 황대선, 고범수 |
| [인기 피드 조회](#popular-feed) | 클라이언트가 level1, level2, level3 게시물을 각각 최대 10개까지 요청. MariaDB에 저장된 게시글들의 개수 확인 (30개 이상 여부). PostRandomFetcher 클래스를 통해 랜덤하게 게시물 선택. Redis의 SortedSet 자료구조를 이용하여 순위 조회. | 황대선 |
| [조회수](#view-count) | Redis의 Set 자료구조를 사용하여 게시글 조회 시 회원의 Primary Key를 저장함으로써, 한 회원이 조회수를 악의적으로 조작하는 것을 방지합니다.<br> 스케줄러를 활용하여 10분마다 Redis의 Set 자료구조에 저장된 데이터를 기반으로 MariaDB에 게시글의 조회수를 업데이트하고, 이후 Set 자료구조의 데이터를 초기화(롤백)합니다 | 황대선 |
| [인기 게시글 선정](#popular-post-selection) | 인기도는 게시글의 연령, 조회수, 그리고 추천 수를 기반으로 하여 10분마다 갱신. Redis의 SortedSet 자료구조에 저장. | 황대선 |
| [폭죽 사용](#firework-use) | 해당 인기 게시글의 레벨을 검증한 후, 그에 해당하는 포인트를 사용. 사용자의 포인트가 충분할 경우, 해당 게시물의 소유자의 인스타그램 아이디 반환. | 황대선, 고범수 |
| [예외 처리](#exception-handling) | RuntimeException 클래스 상속하여 커스텀 예외 생성. GlobalExceptionHander와 PostExceptionHandler 클래스를 이용한 예외 핸들링. MessageCode 클래스를 통해 오류 코드와 설명 제공. | 황대선 |
| [API 응답](#api-response) | ApiResponse 클래스와 ApiResponseGenerator 클래스를 생성하여 API 응답 표준화. | 황대선 |
| [아키텍처 설계](#architecture-design) | Layred Architecture에 따른 설계. Presentation, Application, Infrastructure Layer 및 예외 처리 클래스 구성. | 황대선, 고범수 |
| [각종 컨벤션](#conventions) | 네이밍 및 커밋 컨벤션 정의. | 황대선, 고범수 |
| [카카오 로그인](#kakao-login) | 카카오 OAuth2 Rest API를 사용한 로그인 및 사용자 정보 조회 기능 구현. | 김유선 |
| [인스타그램 연동](#instagram-integration) | Instagram Graph OAuth2 Rest API를 사용한 로그인 및 사용자 정보 조회 기능 구현. | 김유선 |
| [JWT 토큰](#jwt-token) | 사용자 식별 UID 및 권한을 담은 토큰 발급 기능 구현. | 김유선 |
| [토큰 관리](#token-management) | 엑세스토큰과 리프레시토큰 발급 기능 개발. | 김유선 |
| [CORS 설정](#cors-setup) | 프론트배포URL에서만 백엔드서버 API 요청 및 응답 가능하도록 CORS 설정. | 김유선 |
| [접근권한 설정](#access-control) | API 별 접근권한 설정 및 사용자 ROLE 별 기능 제한. | 김유선 |
| [피드 조회 공통 클래스 구현](#feed-common-class) | PostFetchResponse 클래스는 홈피드와 마이피드 등 다양한 피드 조회 기능에서 사용되는 공통 구조를 정의합니다. 이 클래스를 사용함으로써 다양한 피드 조회 시나리오에서 코드 중복을 줄였습니다. | 고범수 |
| [홈 피드](#home-feed) | 사용자 인증 상태에 따라 다른 정보를 제공하는 홈 피드 구현. | 고범수 |
| [자신이 올린 게시물 조회](#personal-post-view) | 커서 기반 무한 스크롤 방식을 사용한 게시물 조회 기능. | 고범수, 송가은 |
| [토글 방식의 좋아요 기능](#like-toggle) | 단일 엔드포인트를 통한 좋아요 토글 기능 구현. | 고범수 |
| [폭죽 사용 기록 추적](#firework-tracker) | 폭죽 사용 기록 추적 및 `instacount` 테이블 도입으로 데이터 처리 개선. | 고범수 |
